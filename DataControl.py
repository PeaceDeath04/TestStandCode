...
class Packet:
    def __init__(self,raw_packet,taring_traction = None,taring_weight = None):
        ...
        self.taring_traction = taring_traction

        self.taring_weight = taring_weight

        # 1. Обьединям наименования ключей с их значениями
        for key, value in zip(self.keys_to_update_ard, raw_packet):
            self.data[key] = float(value)

        # 2. производим тарирования
        self.taring_values(self.data,taring_traction)
        self.taring_values(self.data, taring_weight)

    def taring_values(self,packet_data,dict_tar):
        if dict_tar is not None:
            for key in packet_data:
                packet_data[key] -= dict_tar.get(key, 0)
                if len(dict_tar) >2:
                    _key = key + "2"
                    packet_data -= dict_tar.get(_key,0)
            return packet_data
